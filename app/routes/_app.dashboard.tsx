...
function renderAds(m,pm) {
  ...
  document.getElementById('ad-leaderboard-table').innerHTML = '<table class="ad-table"><thead><tr><th>#</th><th>Ad Creative</th><th>Spend</th><th>Leads</th><th>CPL</th><th>CTR</th><th>Frequency</th></tr></thead><tbody>'+ads.map(function(a,i){return '<tr><td><div class="ad-rank '+(i===0?'top':'')+'">'+(i+1)+'</div></td><td><div class="ad-name">'+a.adName+'</div><div class="ad-adset">'+a.adSetName+'</div></td><td class="ad-metric">'+fmtC(a.spend)+'</td><td class="ad-metric">'+a.leads+'</td><td class="ad-metric '+(i===0?'best':'')+'">'+fmtC(a.cpl)+'</td><td class="ad-metric">'+fmtP(a.uniqueCtr)+'</td><td class="ad-metric">'+a.frequency.toFixed(2)+'</td></tr>';}).join('')+'</tbody></table>';
}
...
function renderROIContent(tab, m, l, pm, pl, numMonths) {
  ...
  var totalBookings = l&&l.booked || 0;
  var confirmedBookings = l&&l.confirmed || 0;
  var cancelledBookings = l&&l.cancelled || 0;
  var upcomingBookings = Math.max(0, totalBookings - confirmedBookings - cancelledBookings);
  var moneyCollected = l&&l.moneyCollected || 0;
  var totalCost = adSpend + agencyFee;
  var clientLTV = getLTV();
  var hasLTV = clientLTV !== null;
  var ltvRevenue = hasLTV ? confirmedBookings * clientLTV : 0;
  ...
  var trueROISub = hasLTV ? 'Based on confirmed bookings x LTV ($'+clientLTV+')' : 'LTV not yet determined';

  tab.innerHTML = '<div class="kpi-grid">'+roiKpi('Agency Fee',fmtCw(agencyFee))+roiKpi('Ad Spend',fmtC(adSpend))+roiKpi('Total Cost',fmtC(totalCost))+roiKpi('Total Bookings',totalBookings)+roiKpi('Confirmed Bookings',confirmedBookings)+roiKpi('Upcoming Bookings',upcomingBookings)+roiKpi('Money Collected',fmtCw(moneyCollected))+roiKpi('LTV Revenue',ltvRevenueDisplay)+'</div><div class="chart-row"><div class="chart-card"><h3>Return on Investment</h3><div class="chart-sub">Based on actual money collected</div><div style="text-align:center;padding:30px 0;"><div style="font-size:48px;font-weight:700;color:'+roiColor+';">'+roi.toFixed(1)+'%</div><div style="font-size:14px;color:#999;margin-top:8px;">ROI</div></div></div><div class="chart-card"><h3>TRUE ROI (Projected)</h3><div class="chart-sub">'+trueROISub+'</div><div style="text-align:center;padding:30px 0;"><div style="font-size:48px;font-weight:700;color:'+trueRoiColor+';">'+trueROIDisplay+'</div><div style="font-size:14px;color:#999;margin-top:8px;">TRUE ROI</div></div></div></div><div class="chart-row full"><div class="chart-card"><h3>ROI Breakdown</h3><div class="chart-sub">Cost vs revenue comparison</div><div class="chart-container"><canvas id="c-roi-bar"></canvas></div></div></div><div class="chart-row full"><div class="chart-card" style="background:var(--beige-light);"><h3 style="margin-bottom:16px;">Metric Definitions</h3><div style="display:grid;gap:12px;font-size:13px;line-height:1.6;"><div><strong>Agency Fee:</strong> The monthly retainer paid to Media Waffle for management and strategy.</div><div><strong>Ad Spend:</strong> Total amount spent on Meta (Facebook/Instagram) advertising for the period.</div><div><strong>Total Cost:</strong> The combined investment: Ad Spend + Agency Fee. This is the total outlay for the period.</div><div><strong>Total Bookings:</strong> All appointments booked from leads during this period (confirmed + upcoming + cancelled).</div><div><strong>Confirmed Bookings:</strong> Appointments that were attended and confirmed as completed.</div><div><strong>Upcoming Bookings:</strong> Appointments that are booked but not yet confirmed or cancelled. Calculated as Total Bookings minus Confirmed minus Cancelled.</div><div><strong>Money Collected:</strong> Revenue generated from all confirmed appointments that came from leads during this period.</div><div><strong>LTV (Lifetime Value):</strong> '+ltvDescription+'</div><div><strong>LTV Revenue:</strong> Projected total revenue: Confirmed Bookings x LTV. Represents the expected long-term value of confirmed appointments this period.</div><div><strong>ROI (Return on Investment):</strong> Money Collected / Total Cost x 100. Shows actual return based on cash already received. Below 100% means you have not recouped costs yet from collected revenue alone.</div><div><strong>TRUE ROI:</strong> LTV Revenue / Total Cost x 100. Based on confirmed bookings only. Shows projected return when accounting for the full lifetime value of each confirmed appointment. This is the more accurate measure of campaign profitability.</div></div></div></div>';
