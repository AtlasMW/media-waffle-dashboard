...
function aggregateDaily(days) {
  var totals = { spend: 0, impressions: 0, reach: 0, leads: 0, messages: 0 };
  days.forEach(function(d) { totals.spend += d.spend; totals.impressions += d.impressions; totals.reach += d.reach; totals.leads += d.leads; totals.messages += d.messages; });
...
function renderOverview(m,l,pm,pl) {
  ...
  tab.innerHTML = '<div class="kpi-grid">'+kpi('spend','Total Spend',fmtC(m.spend),trend(m.spend,pm&&pm.spend))+kpi('leads','Total Leads',m.leads,trend(m.leads,pm&&pm.leads))+kpi('cpl','Cost Per Lead',fmtC(m.cpl),trend(m.cpl,pm&&pm.cpl,true))+kpi('responded','Responded %',l?fmtPw(l.respondedPct):'--',l&&pl?trend(l.respondedPct,pl.respondedPct):'')+kpi('booked','Booked',l?l.booked:'--',l&&pl?trend(l.booked,pl.booked):'')+'</div><div class="chart-row"><div class="chart-card"><h3>Spend vs Leads</h3><div class="chart-sub">Daily trend</div><div class="chart-container"><canvas id="c-spend-leads"></canvas></div></div><div class="chart-card"><h3>Cost Per Lead</h3><div class="chart-sub">Daily trend</div><div class="chart-container"><canvas id="c-cpl"></canvas></div></div></div><div class="chart-row">'+(l?'<div class="chart-card"><h3>Lead Status</h3><div class="chart-sub">Current month breakdown</div><div class="chart-container short"><canvas id="c-lead-status"></canvas></div></div>':'')+'<div class="chart-card"><h3>Best Performing Ad</h3><div class="chart-sub">Highest efficiency this month</div><div style="padding:16px 0;"><div style="font-weight:700;font-size:16px;margin-bottom:4px;">'+(bestAd&&bestAd.adName||'--')+'</div><div style="font-size:12px;color:#999;margin-bottom:16px;">'+(bestAd&&bestAd.campaignName||'')+'</div><div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:16px;"><div><div class="kpi-label">Leads</div><div style="font-size:22px;font-weight:700;margin-top:4px;">'+(bestAd&&bestAd.leads||0)+'</div></div><div><div class="kpi-label">CPL</div><div style="font-size:22px;font-weight:700;margin-top:4px;">'+fmtC(bestAd&&bestAd.cpl||0)+'</div></div><div><div class="kpi-label">Unique CTR</div><div style="font-size:22px;font-weight:700;margin-top:4px;">'+fmtP(bestAd&&bestAd.uniqueCtr||0)+'</div></div></div></div></div></div>'+LEGEND_OVERVIEW;
  ...
  ]},options:{responsive:true,maintainAspectRatio:false,scales:{y:{position:'left',grid:{color:'#f0ebe3'},ticks:{callback:function(v){return '$'+v;}}},y1:{position:'right',grid:{display:false}},x:dailyXAxis()}}});
...
function renderOverviewRange(m,l,pm,pl,days,leads) {
  ...
  tab.innerHTML = '<div class="kpi-grid">'+kpi('spend','Total Spend',fmtC(m.spend),trend(m.spend,pm&&pm.spend))+kpi('leads','Total Leads',m.leads,trend(m.leads,pm&&pm.leads))+kpi('cpl','Cost Per Lead',fmtC(m.cpl),trend(m.cpl,pm&&pm.cpl,true))+kpi('responded','Responded %',fmtPw(l.respondedPct),pl?trend(l.respondedPct,pl.respondedPct):'')+kpi('booked','Booked',l.booked,pl?trend(l.booked,pl.booked):'')+'</div><div class="chart-row"><div class="chart-card"><h3>Daily Spend vs Leads</h3><div class="chart-sub">Daily breakdown</div><div class="chart-container"><canvas id="c-spend-leads"></canvas></div></div><div class="chart-card"><h3>Daily CPL</h3><div class="chart-sub">Cost per lead trend</div><div class="chart-container"><canvas id="c-cpl"></canvas></div></div></div><div class="chart-row"><div class="chart-card"><h3>Lead Status</h3><div class="chart-sub">Period breakdown</div><div class="chart-container short"><canvas id="c-lead-status"></canvas></div></div><div class="chart-card"><h3>Period Summary</h3><div class="chart-sub">Key metrics at a glance</div><div class="period-summary-grid" style="padding:16px 0;display:grid;grid-template-columns:1fr 1fr;gap:20px;"><div><div class="kpi-label">Total Leads</div><div style="font-size:28px;font-weight:700;margin-top:4px;">'+l.totalLeads+'</div></div><div><div class="kpi-label">Responded</div><div style="font-size:28px;font-weight:700;margin-top:4px;">'+l.responded+'</div></div><div><div class="kpi-label">Avg Daily Spend</div><div style="font-size:28px;font-weight:700;margin-top:4px;">'+fmtC(days.length?m.spend/days.length:0)+'</div></div><div><div class="kpi-label">Avg Daily Leads</div><div style="font-size:28px;font-weight:700;margin-top:4px;">'+(days.length?(m.leads/days.length).toFixed(1):'0')+'</div></div></div></div></div>'+LEGEND_OVERVIEW;

  var pr = (currentRange === '90d' || (currentRange==='custom' && _numD > 45)) ? 4 : 3;
  charts.sl = new Chart(document.getElementById('c-spend-leads'),{type:'line',data:{labels:labels,datasets:[
    {label:'Spend ($)',data:chartData.map(function(d){return d.spend;}),borderColor:C.beige,fill:false,pointRadius:pr,pointBackgroundColor:C.beige,tension:0.3,yAxisID:'y',borderDash:[6,4]},
    {label:'Leads',data:chartData.map(function(d){return d.leads;}),borderColor:C.dark,backgroundColor:'rgba(59,59,59,0.06)',fill:true,pointRadius:pr,pointBackgroundColor:C.dark,tension:0.3,yAxisID:'y1'}
  ...
  var cplData = chartData.map(function(d){ return d.leads > 0 ? d.spend / d.leads : null; });
...
function renderAds(m,pm) {
  ...
  charts.camp = new Chart(document.getElementById('c-campaigns'),{type:'bar',data:{labels:campLabels,datasets:[
    {label:'Spend ($)',data:camps.map(function(c){return c.spend;}),backgroundColor:C.beige,borderRadius:4,yAxisID:'y',order:2},
    {label:'Leads',data:camps.map(function(c){return c.leads;}),backgroundColor:C.dark,borderRadius:4,yAxisID:'y1',order:1}
  ]},options:{responsive:true,maintainAspectRatio:false,scales:{y:{position:'left',grid:{color:'#f0ebe3'},ticks:{callback:function(v){return '$'+v;}}},y1:{position:'right',grid:{display:false}}},plugins:{legend:{labels:{usePointStyle:true,pointStyle:'circle'}}}}});
...
function renderLeads(l,pl,m) {
  ...
  var mDaily = (metaData.daily || []).filter(function(d){ return d.date.startsWith(currentMonth); });
  var allLeads = leadsData.allLeads || [];
  var dailyLabels = mDaily.map(function(d){ return parseLocalDate(d.date).getDate(); });
  var dailyLeadCounts = mDaily.map(function(d){ return allLeads.filter(function(l2){ return l2.date === d.date; }).length; });
  var dailySpend = mDaily.map(function(d){ return d.spend; });
  ...
  },options:{responsive:true,maintainAspectRatio:false,scales:{y:{position:'left',grid:{color:'#f0ebe3'}},y1:{position:'right',grid:{display:false},ticks:{callback:function(v){return '$'+v;}}},x:dailyXAxis()}}});
}

function renderLeadsRange(l,pl,leads,days) {
  var tab=document.getElementById('tab-leads');
  var dayMap = {};
  leads.forEach(function(ld){ dayMap[ld.date] = (dayMap[ld.date]||0) + 1; });
  var _numD = currentRange==='custom' && customStart && customEnd ? Math.round((parseLocalDate(customEnd)-parseLocalDate(customStart))/86400000)+1 : 0;
  var bSize = (currentRange === '90d' || (currentRange==='custom' && _numD > 45)) ? 7 : 1;
  var dayLabels, dayLeadCounts, daySpend;
  if (bSize > 1) {
    dayLabels = []; dayLeadCounts = []; daySpend = [];
    for (var i = 0; i < days.length; i += bSize) {
      var chunk = days.slice(i, i + bSize);
      dayLabels.push(fmtBucketLabel(chunk[0].date, chunk[chunk.length-1].date));
      daySpend.push(chunk.reduce(function(s,d){return s+d.spend;}, 0));
      var startD = chunk[0].date, endD = chunk[chunk.length-1].date;
      dayLeadCounts.push(leads.filter(function(l2){return l2.date >= startD && l2.date <= endD;}).length);
    }
  } else {
    dayLabels = days.map(function(d){return fmtDateShort(d.date);});
    dayLeadCounts = days.map(function(d){return dayMap[d.date]||0;});
    daySpend = days.map(function(d){return d.spend;});
  }
  ...
  tab.innerHTML = '<div class="kpi-grid">'+kpi('leads','Total Opportunities',l.totalLeads,trend(l.totalLeads,pl&&pl.totalLeads))+kpi('ctr','Responded %',fmtPw(l.respondedPct),trend(l.respondedPct,pl&&pl.respondedPct))+kpi('cpl','Booked Appointments',l.booked,trend(l.booked,pl&&pl.booked))+kpi('conv','Booking Rate',fmtPw(l.bookedPct),trend(l.bookedPct,pl&&pl.bookedPct))+kpi('revenue','Money Collected',fmtCw(l.moneyCollected),trend(l.moneyCollected,pl&&pl.moneyCollected))+kpi('booked','Confirmed Appointments',l.confirmed,trend(l.confirmed,pl&&pl.confirmed))+kpi('ctr','Cancelled Appointments',l.cancelled,trend(l.cancelled,pl&&pl.cancelled,true))+kpi('ctr','Cancellation Rate',fmtPw(l.cancellationRate||0),trend(l.cancellationRate||0,pl&&pl.cancellationRate||0,true))+kpi('booked','Rebooked Appointments',l.rebooked||0,trend(l.rebooked||0,pl&&pl.rebooked||0))+kpi('conv','Rebooking Rate',fmtPw(l.rebookingRate||0),trend(l.rebookingRate||0,pl&&pl.rebookingRate||0))+'</div><div class="chart-row"><div class="chart-card"><h3>Lead Status Breakdown</h3><div class="chart-sub">Selected period</div><div class="chart-container"><canvas id="c-lead-donut"></canvas></div></div><div class="chart-card"><h3>Daily Lead Volume vs Budget</h3><div class="chart-sub">Leads and spend per day</div><div class="chart-container"><canvas id="c-lead-trend"></canvas></div></div></div>'+(offerCards?'<div class="chart-row full"><div class="chart-card"><h3>Offer Comparison</h3><div class="chart-sub">Budget, leads, CPL, and bookings by offer</div><div class="offer-grid">'+offerCards+'</div></div></div>':'')+'<div class="chart-row full"><div class="chart-card"><h3>Leads in Period</h3><div class="chart-sub">'+leads.length+' leads found</div><table class="lead-table"><thead><tr><th>Date</th><th>Name</th><th>Offer</th><th>Responded</th><th>Status</th></tr></thead><tbody>'+leads.slice().reverse().slice(0,10).map(function(lead){var rc=(lead.responded||'').toLowerCase()==='yes'?'yes':(lead.responded||'').toLowerCase()==='no contact'?'nocontact':'infollowup'; var sc=(lead.status||'').toLowerCase().replace(/\\s/g,''); return '<tr><td>'+fmtDate(lead.date)+'</td><td style="font-weight:600;">'+lead.name+'</td><td>'+lead.offer+'</td><td><span class="status-badge '+rc+'">'+lead.responded+'</span></td><td>'+(lead.status?'<span class="status-badge '+sc+'">'+lead.status+'</span>':'<span style="color:#ccc;">--</span>')+'</td></tr>';}).join('')+'</tbody></table></div></div>'+LEGEND_LEADS;
